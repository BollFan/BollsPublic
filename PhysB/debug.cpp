...
public:
    ...
    float       color[4];
public:
    Sphere(float rrr, float mmm)
		: radius(rrr), mass(mmm), pos(), vel(), acc()
    {
	...
	void setRadiusAndMass(float rr, float mm){
		this->radius = rr;
		this->mass   = mm;
	}
};
...
{
    vec3 s = a->pos - b->pos; // vector between the centers of each sphere
    vec3 v = a->vel - b->vel; // relative velocity between spheres
    float r = a->radius + b->radius;

    float c1 = dot(s,s) - r*r; // if negative, they overlap
...
void sphereCollisionResponse(Sphere *a, Sphere *b)
{
    ...
    float m1, m2, x1, x2;
    vec3 v1temp, v1, v2, v1x, v2x, v1y, v2y, x(a->pos - b->pos);
    ...
    a->vel = vec3( v1x*(m1-m2)/(m1+m2) + v2x*(2*m2)/(m1+m2) + v1y );
    b->vel = vec3( v1x*(2*m1)/(m1+m2) + v2x*(m2-m1)/(m1+m2) + v2y );
}
...
void doGravity()
{
    ...
    for (size_t i=0; i<objects; ++i)
    {
        ...
        vec3 direction = (center - a->pos) / vec3(a->pos.length());
...
int main()
{
    ...
    spheres[3] = new Sphere(1,1);
    spheres[4] = new Sphere(1,1);

    spheres[0]->pos = vec3(0,3,-0.1);
    spheres[1]->pos = vec3(0,-3,0.1);
    spheres[2]->pos = vec3(0,1,5);
    spheres[3]->pos = vec3(0,1,6);
    spheres[4]->pos = vec3(0,1,7);

    spheres[0]->vel = vec3(0,-0.5,0);
    spheres[1]->vel = vec3(0,0.5,0);
    spheres[2]->vel = vec3(0,0,0);
    spheres[3]->vel = vec3(0,0,0);
    spheres[4]->vel = vec3(0,0,0);

    // Colors
    spheres[0]->color[0] = 1.0f;
    spheres[1]->color[1] = 1.0f;
    spheres[2]->color[2] = 1.0f;
    spheres[3]->color[0] = 0.5f;
    spheres[4]->color[0] = 0.5f;
